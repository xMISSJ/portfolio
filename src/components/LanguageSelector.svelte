...
{#if mobileMenu}
  <div
    class="flex items-center gap-1 rounded-full border border-[var(--color-lilac)] bg-[rgba(224,176,255,0.06)] p-1"
  >
    {#each languages as lang}
      <button
        class="flex h-[34px] min-w-[64px] items-center justify-center gap-[6px] rounded-full border border-transparent px-[10px] font-['Inter',sans-serif] text-[12px] text-[var(--color-off-white)] transition-all duration-200 ease-in hover:border-[var(--color-lilac)]"
        class:bg-[var(--color-lilac)]={lang.code === selectedLanguage}
        class:text-[var(--color-background)]={lang.code === selectedLanguage}
